...
OUT_FILE = './twitter.tag_cloudDemocrats.html'
MIN_FREQUENCY = 50
MIN_FONT_SIZE = 3
MAX_FONT_SIZE = 20

comments = c.execute('select tweet from commentsDemocrats')
comments = c.fetchall()
words = c.execute('select word from frequent_words')
words = c.fetchall()
messages = []
for m in comments:
    messages.append(m[0].lower())
